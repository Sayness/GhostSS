...
namespace GhostSS.Scan
{
    class ScanLsass
    {
        ...
        {
            Processus lsass = new Processus("lsass");
            ...
            string path = Dumper.DumpLsass(process);
            ...
            var sw = new StreamWriter(File.OpenWrite(path + "3"));
            ...
            Console.ForegroundColor = ConsoleColor.White;
            using (FileStream fileStream = File.Open(path + "3", FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                            ...
                            foreach (string hash in hashlist.ToList())
                            {
                                if (hash.Equals(line))
                                {
                                    ...
                                    if (!result.Contains(Main.Decrypt(clientfound, 1000)))
                                    {
                                        ...
                                        lsass.results.Add(new Result(Main.Decrypt(clientfound, 1000)));
