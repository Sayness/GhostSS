...
namespace GhostSS.Scan
{
    class ScanDNS
    {
        ...
        public static Processus Scan(string link)
        {
            Processus dns = new Processus("dns");

            Console.ForegroundColor = ConsoleColor.Green;
            Console.Write("[->]");
            Console.ForegroundColor = ConsoleColor.White;
            Console.WriteLine(" Check of dns just launched. ");
            ...
            string path = Dumper.DumpDNS();
            var sr = new StreamReader(File.OpenRead(path));
            var sw = new StreamWriter(File.OpenWrite(path + "4"));
            ...
            using (Stream stream = w.OpenRead(link))
            {
                using (BufferedStream bs = new BufferedStream(stream))
                {
                    using (StreamReader streamReader = new StreamReader(bs))
                    {
                        ...
                        Console.Write("[->]");
                        Console.ForegroundColor = ConsoleColor.White;
                        Console.WriteLine(" Scanning the dns...");
                    }
                }
            }
            ...
            Console.ForegroundColor = ConsoleColor.White;
            using (FileStream fileStream = File.Open(path + "4", FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                            ...
                            foreach (string hash in hashlist.ToList())
                            {
                                if (hash.Equals(line))
                                {
                                    ...
                                    if (!result.Contains(Main.Decrypt(clientfound, 1000)))
                                    {
                                        ...
                                        dns.results.Add(new Result(Main.Decrypt(clientfound, 1000)));
