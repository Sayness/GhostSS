...
namespace GhostSS.Scan
{
    class ScanExplorer
    {
        ...
        {
            DateTime explorer = Utils.GetProcessStartTime("explorer");
            DateTime ctfmon = Utils.GetProcessStartTime("ctfmon");

         
            if (explorer.Minute != ctfmon.Minute)
            {
                Utils.ExplorerRestarted = "Explorer Restarted (at " + explorer + ")";
            }
        ...
        {
            CheckRestart();
            Processus explorer = new Processus("explorer");
            ...
            string path = Dumper.DumpExplorer(process);
            ...
            var sw = new StreamWriter(File.OpenWrite(path + "2"));
            Main.CopyLinesRemovingAllDupes(sr, sw);
            sw.Close();
            sr.Close();
            DeletedFiles.FoundPca();
            ...
            Console.ForegroundColor = ConsoleColor.White;
            using (FileStream fileStream = File.Open(path + "2", FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                            ...
                            foreach (string hash in hashlist.ToList())
                            {
                                if (hash.Equals(line))
                                {
                                    ...
                                    if (!result.Contains(Main.Decrypt(clientfound, 1000)))
                                    {
                                        ...
                                        explorer.results.Add(new Result(Main.Decrypt(clientfound, 1000)));
